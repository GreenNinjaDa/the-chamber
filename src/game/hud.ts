...
.hud-ring { position: absolute; border: 4px solid #ff2a1a; border-radius: 50%; box-sizing: border-box;
            box-shadow: 0 0 8px rgba(255,40,20,.8), inset 0 0 6px rgba(255,40,20,.5);
            animation: hud-pulse .55s ease-in-out infinite; }
.hud-arrow { position: absolute; width: 54px; height: 44px; margin: -22px 0 0 -27px; }
...
export class Hud {
  ...
  markers(list: ScreenMarker[]) {
    ...
    this.markerPool.forEach((m, i) => {
      ...
      m.ring.classList.toggle('purple', purple);
      m.arrow.classList.toggle('purple', purple);
      if (t.onScreen) {
