...
export class Scoreboard {
  ...
  blink = false;
  /** The machine has TILTed: the bulbs all flash. */
  tilt = false;
  ...
  private bulbs: { item: DrawItem; lit: number[]; dim: number[] }[] = [];
  private lastScore = -1;
  private lastTime = -1;
  ...
  constructor() {
    ...
    const bulb = (x: number, y: number) => {
      const c = BULB[this.bulbs.length % 2];
      const item: DrawItem = { mesh: 'sphere', model: mul(translation([x, y, Z + 0.1]), scaling([0.13, 0.13, 0.13])), color: c, pattern: Pattern.emissive, shadow: false };
      this.bulbs.push({ item, lit: c, dim: [c[0] * 0.15, c[1] * 0.15, c[2] * 0.15] });
    };
  ...
  draw(out: DrawItem[], time: number) {
    ...
    // Chasing bulbs round the frame (all flashing when the machine TILTs).
    const chase = Math.floor(time * 6);
    for (let k = 0; k < this.bulbs.length; k++) {
      const b = this.bulbs[k];
      b.item.color = this.on < 1 ? b.dim : this.tilt ? (Math.sin(time * 14) > 0 ? BULB[1] : b.dim) : (k + chase) % 3 !== 0 ? b.lit : b.dim;
      out.push(b.item);
    }
    if (this.appear < 1) {
      // Unfolding: squashed flat against the wall to start with, then springing out (on copies:
      // the items above are reused every frame).
      ...
      for (let i = first; i < out.length; i++) out[i] = { ...out[i], model: mul(m, out[i].model) };
    }
